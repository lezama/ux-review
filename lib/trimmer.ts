...
export function trimVideo(
	...
	try {
		execSync(
			`ffmpeg -i ${ JSON.stringify( inputPath ) } -vf "freezedetect=n=-60dB:d=${ freezeThreshold }" -f null - 2>&1`,
			{ encoding: 'utf8' }
		);
		stderr = '';
	} catch ( e: unknown ) {
		// ffmpeg returns non-zero but we still get output
		stderr = ( e as { stdout?: string } ).stdout ?? '';
	}
	...
	try {
		...
		for ( let i = 0; i < validSegments.length; i++ ) {
			...
			execSync(
				`ffmpeg -y -ss ${ start.toFixed( 3 ) } -to ${ end.toFixed( 3 ) } -i ${ JSON.stringify( inputPath ) } ${ ENCODE_PRESET } -an ${ JSON.stringify( segFile ) }`,
				{ stdio: 'ignore' }
			);
		}
		...
		execSync(
			`ffmpeg -y -f concat -safe 0 -i ${ JSON.stringify( concatFile ) } ${ ENCODE_PRESET } -an ${ JSON.stringify( outputPath ) }`,
			{ stdio: 'ignore' }
		);

		const trimmedDuration = validSegments.reduce(
			( sum, [ s, e ] ) => sum + ( e - s ),
			0
		);
