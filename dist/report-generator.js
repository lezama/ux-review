...
export function generateReport(options) {
    ...
    const loggedFiles = new Set(screenshots
    ...
    for (const entry of screenshots) {
        if (entry.screenshotFile) {
            const fullPath = path.join(options.screenshotDir, entry.screenshotFile);
    ...
    for (let i = 0; i < sceneMarkers.length; i++) {
        ...
        const sceneScreenshots = screenshots.filter((s) => s.timestampMs >= scene.timestampMs &&
            s.timestampMs < endMs);
        let status = '\u2713';
        const framesPerSec = durationSec > 0
    ...
    if (issues.length > 0) {
        ...
        for (const issue of issues) {
            const sceneRef = issue.scene
                ? ` Scene ${issue.scene}:`
                : '';
            lines.push(`  [${issue.severity}]${sceneRef} ${issue.message}`);
        }
    }
