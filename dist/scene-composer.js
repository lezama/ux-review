...
export class SceneComposer {
    ...
    static composeFromSegments(options) {
        ...
        const tmpDir = path.join(path.dirname(outputPath), '.scene-tmp');
...
function generateSceneAudio(scenes, tmpDir) {
    ...
    for (let i = 0; i < scenes.length; i++) {
        ...
        if (scene.narration) {
            const aiffPath = path.join(tmpDir, `narr-${i}.aiff`);
            ...
            if (remainingSilence > 0.1) {
                const silPath = path.join(tmpDir, `sil-${i}.aiff`);
...
function generateSceneSRT(scenes, tmpDir, transitionSec) {
    ...
    for (let i = 0; i < scenes.length; i++) {
        ...
        if (scene.narration) {
            ...
            const displayDuration = Math.min(duration * 0.8, duration - 0.3);
            const endMs = Math.round((cumulativeOffset + displayDuration) * 1000);
...
function joinScenes(scenePaths, transitionSec, tmpDir) {
    ...
    execSync(`ffmpeg -y ${inputs} -filter_complex "${filterComplex}" -map "[vout]" ${ENCODE_PRESET} -an ${JSON.stringify(outputFile)}`, { stdio: 'ignore', timeout: 300_000 });
    return outputFile;
}
...
//# sourceMappingURL=scene-composer.js.map