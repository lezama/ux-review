 ...
 */
export function concatenateAudio(files, outputPath) {
    if (files.length === 0) {
        return;
    }
    if (files.length === 1) {
 ...
 */
export function isDualPersonaLayout(layout) {
 ...
 */
export function generateSpeech(text, outputPath, voice) {
    const engine = detectTTSEngine();
    if (engine === 'qwen') {
        const voiceArg = voice ? `-v ${JSON.stringify(voice)}` : '';
        execSync(`python3.11 ${JSON.stringify(QWEN_SCRIPT)} ${voiceArg} -o ${JSON.stringify(outputPath)} ${JSON.stringify(text)}`, { stdio: ['ignore', 'pipe', 'ignore'], timeout: 60_000 });
    }
 ...
 */
export function generateSpeechBatch(items) {
    if (items.length === 0) {
        return [];
    }
    const engine = detectTTSEngine();
